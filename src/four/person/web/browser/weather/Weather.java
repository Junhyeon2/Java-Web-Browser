...
public class Weather extends HBox implements Initializable{	
	...
	@FXML HBox hboxDailyStatus;
	@FXML HBox hboxHourlyTopStatus;
	@FXML HBox hboxHourlyBottomStatus;
	...
	File file;
	public Weather() {
		URL urlXLSX = this.getClass().getClassLoader().getResource("four/person/web/browser/data/zipdb.xlsx");
		file = new File(urlXLSX.getPath());
		FXMLLoader fxmlLoader = new FXMLLoader(getClass().getResource("weather.fxml"));
		this.getStylesheets().add(getClass().getResource("weather.css").toExternalForm());
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		hBox.setStyle("-fx-background-image: url('//res/weather/default.jpg');");
		getSido();
	...
	private void setGunguBox(ActionEvent event) {
		...
		if( !comboSido.getValue().equals("세종") ){
			...
			dialog.getDialogPane().setContentText("서비스 준비 중입니다.");
	...
	public void getLatLon(String loc) throws Exception {
		...
		System.out.println("\nSending 'GET' request to URL : " + locationurl); 
	...
	public void getPresentWeather(float lat, float lon) throws Exception {
		...
		hBox.setStyle("-fx-background-image: url('//res/background/"+strWeatherIcon+".jpg');");
		//System.out.println("-fx-background-image: url('//res/background/"+strWeatherIcon+".jpg');");
	...
	public void getDailyWeather(double lat, double lon) throws Exception {
		...
		hboxDailyStatus.getChildren().clear();
		...
		for( int b=1 ; b<6 ; b++ ){
			...
			hboxDailyStatus.getChildren().add(daily);
	...
	public void getHourlyWeather(float lat, float lon) throws Exception {
		...
		hboxHourlyTopStatus.getChildren().clear();
		hboxHourlyBottomStatus.getChildren().clear();
		...
		for( int b=0 ; b<10 ; b++ ){
			...
			if( b<5 ){
				...
				hboxHourlyTopStatus.getChildren().add(hourly);
				
			} else if ( b>=5 ){
				
				hboxHourlyBottomStatus.getChildren().add(hourly);
	...
	public String getHourly( long dt ){
		...
		if( simpleDateFlag.format(time*1000).equals("00") ){
